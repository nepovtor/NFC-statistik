...
from .cookies import cookie_name_for_scope
...
def _load_session(request: Request, scope: str) -> Optional[dict]:
    cookie_value = request.cookies.get(cookie_name_for_scope(scope), "").strip()
    if not cookie_value:
        return None
    ...
    if session["principal_type"] == SESSION_PRINCIPAL_ADMIN and (
        not session["admin_id"] or int(session["admin_is_active"] or 0) != 1
    ):
        close_connection(conn)
        return None
    if session["principal_type"] == SESSION_PRINCIPAL_CLIENT and (
        not session["client_id"] or int(session["client_is_active"] or 0) != 1
    ):
        close_connection(conn)
        return None
